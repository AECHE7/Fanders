...
$penaltyService = new ReportService();
...
if ($generatePdf) {
    ...
    $pdfContent = $penaltyService->generatePenaltiesReport($filters, true);
    if (empty($pdfContent)) {
        echo "Error: PDF content is empty.";
        exit;
    }
    header('Content-Type: application/pdf');
    header('Content-Disposition: inline; filename="penalties_report.pdf"');
...
$reportData = $penaltyService->generatePenaltiesReport($filters, false);
...
<select name="is_paid" id="is_paid" class="form-select">
    ...
    <option value="0" <?= isset($filters['is_paid']) && $filters['is_paid'] === '0' ? 'selected' : '' ?>>Unpaid</option>
    <option value="1" <?= isset($filters['is_paid']) && $filters['is_paid'] === '1' ? 'selected' : '' ?>>Paid</option>
</select>
                    </div>
