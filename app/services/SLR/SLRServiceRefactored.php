...
class SLRServiceRefactored {
    private $legacyService;

    public function __construct() {
        // Use the existing service for now to maintain behavior
        $this->legacyService = new \SLRService();
    }

    /**
     * Generate SLR and return SLRResult
     * @param int $loanId
     * @param int $generatedBy
     * @param string $trigger
     * @return SLRResult
     */
    public function generateSLR($loanId, $generatedBy, $trigger = SLRConstants::TRIGGER_MANUAL) : SLRResult {
        // Normalize trigger to legacy expectations if needed
        $legacyTrigger = $this->mapTriggerToLegacy($trigger);

        try {
            $result = $this->legacyService->generateSLR($loanId, $generatedBy, $legacyTrigger);
            if ($result === false) {
                $err = $this->legacyService->getErrorMessage() ?? 'Unknown error';
                return SLRResult::failure($err);
            }
            return SLRResult::success($result);
        } catch (\Exception $e) {
            return SLRResult::failure('Exception: ' . $e->getMessage());
        }
    }

    public function downloadSLR($slrId, $userId, $reason = ''): SLRResult {
        try {
            $result = $this->legacyService->downloadSLR($slrId, $userId, $reason);
            if ($result === false) {
                $err = $this->legacyService->getErrorMessage() ?? 'Unknown error';
                return SLRResult::failure($err);
            }
    ...
    public function getSLRByLoanId($loanId) {
        return $this->legacyService->getSLRByLoanId($loanId);
    }

    public function getSLRById($slrId) {
        return $this->legacyService->getSLRById($slrId);
    }

    public function listSLRDocuments($filters = [], $limit = 20, $offset = 0) {
        return $this->legacyService->listSLRDocuments($filters, $limit, $offset);
    }

    public function archiveSLR($slrId, $userId, $reason = ''): SLRResult {
        try {
            $ok = $this->legacyService->archiveSLR($slrId, $userId, $reason);
            if ($ok === false) {
                $err = $this->legacyService->getErrorMessage() ?? 'Unknown error';
                return SLRResult::failure($err);
            }
            return SLRResult::success(true);
        } catch (\Exception $e) {
            return SLRResult::failure('Exception: ' . $e->getMessage());
        }
    }

    private function mapTriggerToLegacy(string $trigger): string {
        // Map normalized triggers back to values used by legacy code
        switch ($trigger) {
            case SLRConstants::TRIGGER_LOAN_APPROVAL:
                return 'loan_approval';
            case SLRConstants::TRIGGER_LOAN_DISBURSEMENT:
                return 'loan_disbursement';
            case SLRConstants::TRIGGER_MANUAL:
            default:
